  ...
  <q-page class="q-pa-md column no-wrap overflow-hidden bg-grey-2" style="min-height: unset !important; height: calc(100vh - 95px); max-height: calc(100vh - 95px);">
    ...
    <div class="col-auto row items-center justify-between q-mb-md">
      <div class="row items-center q-gutter-sm">
        <div>
          <div class="row items-center q-gutter-sm">
            <div class="text-h5 text-primary text-weight-bold">Listado de Inventario</div>
          ...
          <div class="text-caption text-grey-7">Visualización general de existencias (public.view_inventario)</div>
        </div>
      </div>
      ...
      <div class="row items-center q-gutter-xs">
        ...
        >
          <q-tooltip>Descargar listado actual en CSV</q-tooltip>
        </q-btn>
        <q-btn
          flat
          round
    ...
    <q-card flat class="col column no-wrap overflow-hidden rounded-borders bg-white shadow-1">
      <q-table
        ...
        :loading="loading"
        no-data-label="No se encontraron registros de inventario"
...
async function fetchData() {
  ...
  try {
    ...
    const res = await inventarioApi.getListado({
    ...
    $q.notify({
      type: 'negative',
      message: 'Error al cargar el listado de inventario',
      position: 'top',
    });
  } finally {
    loading.value = false;
