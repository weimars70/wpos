<template>
  <q-page padding>
    <div class="row items-center justify-between q-mb-md">
      <div class="text-h5 text-primary text-weight-bold">Documentos</div>
      <q-btn color="primary" icon="add" label="Agregar Documentos" @click="openNew" class="shadow-1 q-px-md rounded-borders"/>
    </div>
    ...
    <q-dialog v-model="showForm" persistent>
      <q-card style="min-width: 400px; border-radius: 12px;">
        <q-card-section class="bg-primary text-white row items-center">
          <div class="text-h6">{{ isEditing ? 'Editar' : 'Nuevo' }} Documentos</div>
          <q-space />
        ...
        <q-card-section class="q-pt-md">
          <q-form @submit="saveData" class="q-gutter-md">
            ...
            <q-input v-model="formData.abreviado" label="Abreviado" outlined dense  />
...
function editRow(row: any) {
  ...
  formData.abreviado = row.abreviado;
  showForm.value = true;
}
...
function deleteRow(row: any) {
  $q.dialog({
    ...
    try {
      await api.delete(`/api/documentos/${row.codigo}`);
...
async function saveData() {
  ...
  try {
    const payload = {
      ...
      abreviado: formData.abreviado
    };
    if (isEditing.value) {
      await api.put(`/api/documentos/${formData.codigo}`, payload);
