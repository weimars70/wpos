<template>
  <q-page padding>
    <div class="row items-center justify-between q-mb-md">
      <div class="text-h5 text-primary text-weight-bold">Colores</div>
      <q-btn color="primary" icon="add" label="Agregar Colores" @click="openNew" class="shadow-1 q-px-md rounded-borders"/>
    </div>
    ...
    <q-dialog v-model="showForm" persistent>
      <q-card style="min-width: 400px; border-radius: 12px;">
        <q-card-section class="bg-primary text-white row items-center">
          <div class="text-h6">{{ isEditing ? 'Editar' : 'Nuevo' }} Colores</div>
          <q-space />
...
function deleteRow(row: any) {
  $q.dialog({
    ...
    try {
      await api.delete(`/api/colores/${row.id}`);
...
async function saveData() {
  ...
  try {
    ...
    if (isEditing.value) {
      await api.put(`/api/colores/${formData.id}`, payload);
      $q.notify({ type: 'positive', message: 'Registro actualizado' });
    } else {
      await api.post(`/api/colores`, payload);
