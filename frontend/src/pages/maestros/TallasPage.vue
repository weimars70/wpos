<template>
  <q-page padding>
    <div class="row items-center justify-between q-mb-md">
      <div class="text-h5 text-primary text-weight-bold">Tallas</div>
      <q-btn color="primary" icon="add" label="Agregar Talla" @click="openNew" class="shadow-1 q-px-md rounded-borders"/>
    </div>
    ...
    <q-dialog v-model="showForm" persistent>
      <q-card style="min-width: 400px; border-radius: 12px;">
        <q-card-section class="bg-primary text-white row items-center">
          <div class="text-h6">{{ isEditing ? 'Editar' : 'Nueva' }} Talla</div>
          <q-space />
        ...
        <q-card-section class="q-pt-md">
          <q-form @submit="saveData" class="q-gutter-md">
            <q-input v-model="formData.codigo" label="Código" outlined dense :disable="isEditing" lazy-rules :rules="[val => !!val || 'Obligatorio']" />
...
function deleteRow(row: any) {
  $q.dialog({
    ...
    try {
      await api.delete(`/api/tallas/${row.codigo}`);
...
async function saveData() {
  ...
  try {
    ...
    if (isEditing.value) {
      await api.put(`/api/tallas/${formData.codigo}`, payload);
      $q.notify({ type: 'positive', message: 'Registro actualizado' });
    } else {
      await api.post(`/api/tallas`, payload);
