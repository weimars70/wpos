<template>
  <q-page padding>
    <div class="row items-center justify-between q-mb-md">
      <div class="text-h5 text-primary text-weight-bold">Grupos Items</div>
      <q-btn color="primary" icon="add" label="Agregar Grupos Items" @click="openNew" class="shadow-1 q-px-md rounded-borders"/>
    </div>
    ...
    <q-dialog v-model="showForm" persistent>
      <q-card style="min-width: 400px; border-radius: 12px;">
        <q-card-section class="bg-primary text-white row items-center">
          <div class="text-h6">{{ isEditing ? 'Editar' : 'Nuevo' }} Grupos Items</div>
          <q-space />
...
async function fetchRows() {
  loading.value = true;
  try {
    const response = await api.get('/api/grupos-items');
...
function deleteRow(row: any) {
  $q.dialog({
    ...
    try {
      await api.delete(`/api/grupos-items/${row.codigo}`);
...
async function saveData() {
  ...
  try {
    ...
    if (isEditing.value) {
      await api.put(`/api/grupos-items/${formData.codigo}`, payload);
      $q.notify({ type: 'positive', message: 'Registro actualizado' });
    } else {
      await api.post(`/api/grupos-items`, payload);
